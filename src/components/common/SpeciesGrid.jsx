...
/**
 * Grilla de selección de especies con filtro por texto y soporte multi/mono selección.
 *
 ...
 * 2) Filtra por `textoBusqueda` comparando con nombre común/científico normalizados.
 ...
 *
 * @example
 * <GrillaEspecies especies={db.especies} selectedIds={[1,2]} onChange={setIds} multi columns={3} />
 *
 * Notas de mantenimiento:
 ...
 */
export default function GrillaEspecies({ especies, selectedIds, onChange, multi = true, columns = 3, maxHeight = 380 }) {
  const [textoBusqueda, establecerTextoBusqueda] = useState('')

  const especiesOrdenadas = useMemo(() => {
    const arregloEspecies = Array.isArray(especies) ? especies : []
    return arregloEspecies.slice().sort((a, b) => String(a?.com || '').localeCompare(String(b?.com || '')))
  }, [especies])

  const especiesFiltradas = useMemo(() => {
    const claveBusqueda = normalizarTexto(textoBusqueda)
    if (!claveBusqueda) return especiesOrdenadas
    return especiesOrdenadas.filter((e) => normalizarTexto(e?.com).includes(claveBusqueda) || normalizarTexto(e?.sci).includes(claveBusqueda))
  }, [especiesOrdenadas, textoBusqueda])

  const idsSeleccionados = useMemo(() => {
    const arregloIds = Array.isArray(selectedIds) ? selectedIds : []
    return new Set(arregloIds.map(Number).filter((x) => Number.isFinite(x)))
  }, [selectedIds])
  ...
  const alternarSeleccion = (id) => {
    ...
    const siguiente = new Set(idsSeleccionados)
    if (siguiente.has(idNumerico)) siguiente.delete(idNumerico)
    else siguiente.add(idNumerico)
    onChange?.([...siguiente])
  }
   ...
   * 3) Llama a `alternarSeleccion(id)`.
   *
   * Dependencias externas:
   * - `alternarSeleccion` (helper local).
   *
   * Efectos secundarios:
   ...
   */
  const manejarTeclaTarjeta = (ev, id) => {
   ...
   * 2) Actualiza el estado `textoBusqueda`, lo que recalcula `especiesFiltradas`.
   *
   * Dependencias externas:
   * - `establecerTextoBusqueda` (estado local).
   *
   * Efectos secundarios:
   * - Cambia estado local.
   *
   ...
   *
   * @example
   * <input value={textoBusqueda} onChange={manejarCambioBusqueda} />
   *
   * Notas de mantenimiento:
   * - Si se agrega debounce, implementarlo aquí.
   */
  const manejarCambioBusqueda = (e) => establecerTextoBusqueda(e.target.value)
    ...
    <div style={{ overflow: 'auto', maxHeight, border: '1px solid var(--border)', borderRadius: 10, padding: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fit, minmax(${Math.max(140, Math.round(480 / Math.max(1, Number(columns) || 3)))}px, 1fr))`, gap: 10 }}>
        {especiesFiltradas.map((e) => {
          const idEspecie = Number(e?.id)
          const estaSeleccionada = idsSeleccionados.has(idEspecie)
          return (
            <div
              key={e.id}
              role="button"
              ...
              onKeyDown={(ev) => manejarTeclaTarjeta(ev, e.id)}
              style={{
                border: `1px solid ${estaSeleccionada ? 'rgba(10,143,126,.45)' : 'var(--border)'}`,
                background: estaSeleccionada ? 'var(--teal-lt)' : 'var(--bg)',
                borderRadius: 10,
                padding: '10px 12px',
                minHeight: 62,
                display: 'flex',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
                <div style={{ fontWeight: 800, color: estaSeleccionada ? 'var(--teal)' : 'var(--text)', lineHeight: 1.1 }}>
                  {e?.com || '—'}
                </div>
                {estaSeleccionada ? (
