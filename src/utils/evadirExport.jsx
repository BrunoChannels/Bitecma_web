...
function registerEvadirExport() {
  if (window.exportEvadirXlsx) return

  window.exportEvadirXlsx = async function exportEvadirXlsx(opId) {
    const DB = window.__DB__ || window.DB
    const ESPECIES = window.__ESPECIES__ || window.DB?.especies || []
    ...
    try {
      ...
      const applySheetStyles = (ws, aoa, opts = {}) => {
        const ref = ws['!ref']
        if (!ref) return
        ...
        const isAreaHeader = (h) => /^AREA /i.test(h)
        const intFmt = '0'
        ...
        for (let r = range.s.r; r <= range.e.r; r++) {
          for (let c = range.s.c; c <= range.e.c; c++) {
            ...
            if (opts.icCol != null && c === opts.icCol) cell.s = icStyle
            else cell.s = bodyStyle
      ...
      const colArea = densHeader.indexOf(areaColName)
      ...
      ;(op.botes || []).forEach((b) => {
        ;(b.transectos || []).forEach((t) => {
          ...
          allSpIds.forEach((id, idx) => {
            ...
            row.push({ t: 'n', f: `IFERROR(${countRef}/${areaRef},0)`, v: dens, z: '0.0000' })
          })
      ...
      const addSheetFromRows = (sheetBaseName, header, rows, buildRowFn, addIc) => {
        ...
        applySheetStyles(ws, aoa, addIc ? { icCol: header.length - 1 } : {})
...
export default function EvadirExportBridge() {
  useEffect(() => {
    registerEvadirExport()
  }, [])
  return null
}
