...
(() => {
      ...
      global.innerHTML = `
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:10px">
          <div>
            ...
            <div style="font-size:12px;color:var(--text3);margin-top:2px">Busca en todas las regiones por matrícula, RPA o caleta. Presiona ENTER para mostrar resultados.</div>
          </div>
          <button class="btn b-out b-sm" onclick="mbGlobalClear()">Limpiar</button>
        </div>
        <input class="flt" id="mb-global-q" placeholder="Ej: 401, 100, CAVANCHA..." style="width:100%"
  ...
  window.mbGlobalSearch = function mbGlobalSearch() {
    ...
    const results = botesAll.filter((b) => {
      return normText(b?.nrpa).includes(q) || normText(b?.nmatricula).includes(q) || normText(b?.caleta).includes(q);
    }).slice(0, 150);
    results.sort((a, b) => {
          ...
          <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;justify-content:space-between;width:100%">
            ...
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
              <div style="display:flex;align-items:center;gap:6px">
                ...
                <input type="number" class="cq-count-inp" id="cq_inp_${opId}_${boteId}_${t.num}" value="${n > 0 ? n : ''}" min="0" ${cuadSpId ? '' : 'disabled'} style="width:80px;padding:4px 7px;border:1.5px solid var(--border);border-radius:6px;font-size:12px;font-family:var(--ff-m);text-align:center;font-weight:700;background:var(--bg);color:var(--text);outline:none"
