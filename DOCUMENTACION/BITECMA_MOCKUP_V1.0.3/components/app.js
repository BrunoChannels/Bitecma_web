...
(() => {
  ...
  const DB = (window.DB && Array.isArray(window.DB.operaciones))
    ? window.DB
    : ((window.DB_DATA && Array.isArray(window.DB_DATA.operaciones)) ? window.DB_DATA : { operaciones: [] });
  DB.operaciones = (DB.operaciones || []).filter((o) => o?.id === 'OP-2024-007' || o?.id === 'OP-2026-002');
          ...
          <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;justify-content:space-between;width:100%">
            ...
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
              <div style="display:flex;align-items:center;gap:6px">
                <span style="font-size:11px;color:var(--text3)">Cantidad:</span>
                <input type="number" id="cq_inp_${opId}_${boteId}_${t.num}" value="${n > 0 ? n : ''}" min="0" ${cuadSpId ? '' : 'disabled'} style="width:80px;padding:4px 7px;border:1.5px solid var(--border);border-radius:6px;font-size:12px;font-family:var(--ff-m);text-align:center;font-weight:700;background:#fff;outline:none"
                ...
                <div style="text-align:right">
                  <div id="den_${opId}_${boteId}_${t.num}_${cuadSpId || 'none'}" style="font-family:var(--ff-m);font-size:11px;color:var(--teal);font-weight:700">${cuadSpId ? ((n || 0) / t.area).toFixed(4) : '—'}</div>
                  <div style="font-size:9px;color:var(--text3)">ind/m²</div>
                </div>
              </div>
              <div style="display:flex;gap:6px;align-items:center">
  ...
  window.updateCuadArea = function updateCuadArea(opId, boteId, txNum, val) {
    ...
    if (spId) {
      ...
      if (densEl) densEl.textContent = (n / area).toFixed(4);
    }
  ...
  window.openInitBotes = function openInitBotes(opId) {
    ...
    const row = (zona) => `<tr class="ib-row">
      ...
      <td><input class="ii ib-nom" placeholder="Nombre bote" style="width:140px"></td>
    ...
    <div style="max-height:420px;overflow:auto;border:1px solid var(--border);border-radius:10px">
      <table class="tbl" style="margin:0">
        <thead><tr><th>#</th><th>Zona Muestreo</th><th>Bote</th><th>Buzo</th><th>Densidad Tipo</th><th></th></tr></thead>
  ...
  window.openEditOp = function openEditOp(opId) {
    ...
    <div class="i2">
      ...
      <div class="ig"><label class="il">Nombre organización</label><input class="ii" id="op-org" value="${esc(op.org || '')}"></div>
    </div>
  ...
  window.saveEditOp = function saveEditOp(opId) {
    ...
    op.sector = sector;
    op.tipoOrg = tipoOrg;
    op.org = document.getElementById('op-org')?.value;
    closeMo();
  ...
  window.openIngresarMuestras = function openIngresarMuestras(opId, boteId, spId) {
    ...
    <div class="lp-input-row">
      <div class="ig"><label class="il">${esAlga ? 'Diámetro disco (cm)' : 'Longitud (mm)'}</label>
        <input class="ii" id="${esAlga ? 'inp-d' : 'inp-l'}" type="number" placeholder="${esAlga ? 'ej. 12.4' : 'ej. 104'}" min="0" max="99999" step="${esAlga ? '0.1' : '1'}" autofocus
          style="font-size:17px;font-weight:700;text-align:center"
          onkeydown="if(event.key==='Enter'){${esAlga ? `addLP('${opId}','${boteId}',${spId})` : `document.getElementById('inp-p')?.focus();`}}">
      </div>
  ...
  window.openEditLP = function openEditLP(opId, boteId, spId, idx, reopen = 0) {
    ...
    <div class="i2">
      <div class="ig"><label class="il">${esAlga ? 'Diámetro disco (cm)' : 'Longitud (mm)'}</label><input class="ii" id="ed-l" type="number" min="0" max="99999" step="${esAlga ? '0.1' : '1'}" value="${esAlga ? (m.d ?? m.l ?? '') : m.l}"></div>
      ${esAlga ? '' : `<div class="ig"><label class="il">Peso (g)</label><input class="ii" id="ed-p" type="number" min="0" max="99999" value="${pesoVal}"></div>`}
    </div>
    <div style="display:flex;gap:8px;margin-top:14px">
